...
function DFS(rootId)
{
    ...
    for (k in edges._data)
    {
        ...
        {
            edges._data[k].color = {color:color1};
        }
        else
        {
        ...
        if (((edges._data[k].to == rootId) && (edges._data[k].from == bestCycle.path[bestCycle.nbNodes-1])) || ((edges._data[k].from == rootId) && (edges._data[k].to == bestCycle.path[bestCycle.nbNodes-2]))) edges._data[k].color = {color:color1};
...
function findHamiltonianCycle()
{
    ...
    addNodeLine("Bruteforce",bestCycle.cost,t1-t0,color1);
    addNodeLine("Glouton",cycle.cost,t2-t1,color2);
}